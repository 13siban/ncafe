...
- [x] **DB 테이블 설계** — `notice_popups`
  - `id` (PK), `title`, `content` (TEXT/HTML), `image_url` (nullable)
  - `is_active` (boolean), `start_date`, `end_date` (nullable, 기간 제한용)
  - `created_at`, `updated_at`
- [x] **JPA Entity** — `NoticePopupJpaEntity`
- [x] **Repository** — `NoticePopupJpaRepository`
- [x] **도메인 모델** — `NoticePopup`
- [x] **UseCase** — `ManageNoticePopupUseCase`
  - `createPopup()`, `updatePopup()`, `deletePopup()`
  - `getAllPopups()`, `getActivePopups()` (현재 활성 + 기간 내)
- [x] **Service** — `AdminNoticePopupService` implements UseCase
- [x] **Controller** — `AdminNoticePopupController` (`/admin/notice-popups`)
...
- [x] **공개 API Controller** — `NoticePopupController`
  - `GET /notice-popups/active` — 현재 활성 팝업 목록 (사용자 접속 시 호출)
- [x] **DTO** — `CreateNoticePopupRequest`, `UpdateNoticePopupRequest`, `NoticePopupResponse`

### FE 체크리스트

- [x] **어드민 페이지** — `/admin/notice-popups`
  - 팝업 목록 (활성/비활성 상태 표시)
  - 팝업 생성/수정 폼 (제목, HTML 내용, 이미지, 활성 여부, 기간 설정)
  - 팝업 삭제 기능
  - 미리보기 기능
- [x] **사용자 모달 컴포넌트** — `NoticePopupModal`
  - 전역 레이아웃에 배치 (어느 페이지든 동작)
  - `GET /notice-popups/active` 호출하여 활성 팝업 확인
  - "오늘 하루 안 보기" 버튼 → `localStorage`에 날짜 기록
  - "닫기" 버튼
  - 여러 팝업이 있을 경우 순차 표시 또는 캐러셀
- [x] **어드민 사이드바/네비게이션에 메뉴 추가**

### 테스트 TODO

- [x] 팝업 CRUD API 정상 동작 확인
- [x] 활성 팝업 필터링 (is_active + 기간) 정상 동작
- [x] 비로그인 사용자도 공지 팝업 확인 가능
- [x] "오늘 하루 안 보기" 작동 확인 (localStorage)
- [x] 활성 팝업 없을 시 모달 미노출 확인
- [x] 복수 팝업 동시 활성 시 정상 표시 확인
- [x] 어드민 권한 없는 사용자가 관리 API 접근 불가 확인
...
- [x] **DB 컬럼 추가** — `grade_settings` 테이블
  - `main_color` (VARCHAR, default `#333333`)
  - `text_color` (VARCHAR, default `#FFFFFF`)
- [x] **JPA Entity 수정** — `GradeSettingsJpaEntity`에 `mainColor`, `textColor` 필드 추가
- [x] **도메인 모델 수정** — `GradeSettings`에 컬러 필드 추가
- [x] **DTO 수정** — `CreateGradeSettingsRequest`, 등급 응답 DTO에 컬러 필드 포함
- [x] **Service 수정** — `AdminGradeSettingsService`에서 컬러 저장/반환 로직
- [x] **공개 API** — 등급 목록 공개 조회 (About 페이지용)
...
- [x] 등급 설정에서 컬러 저장/수정 정상 동작
- [x] 마이페이지 등급 카드에 컬러 동적 적용 확인
- [x] About 페이지에서 등급 목록 정상 렌더링
- [x] About 페이지에서 각 등급별 고유 컬러 테마 적용 확인
- [x] 등급 시스템 비활성 시 About 페이지에 등급 섹션 미노출 확인
- [x] 등급 시스템 비활성 시 `/grades/public` API 호출 없음 확인
- [x] 컬러 미설정 등급의 기본 컬러 fallback 확인
...
- [x] **SSE(Server-Sent Events) 엔드포인트** 또는 **Polling 엔드포인트**
...
- [x] 새 주문 생성 시 SSE/폴링으로 알림 수신 확인
- [x] 벨 아이콘 깜빡임 애니메이션 정상 동작
- [x] 토스트 메시지 표시 및 자동 사라짐 확인
- [x] 벨 아이콘 클릭 시 주문 페이지 이동 + 알림 리셋
- [x] SSE 연결 끊김 후 재연결 확인
- [x] 어드민이 아닌 사용자는 SSE 구독 불가 확인
- [x] 복수 어드민 동시 접속 시 모두 알림 수신 확인
- [x] 브라우저 탭 전환 후에도 알림 정상 수신 확인
...
- [x] **드래그앤드롭** — 리스트 뷰에서도 행 드래그로 순서 변경
  - 드래그 핸들 (☰ 아이콘) 좌측 배치
- [x] **Batch 저장** — 여러 항목 수정 시 "변경사항 저장" 버튼 표시
  - 수정된 행 하이라이트
  - 저장 시 batch API 호출

### 테스트 TODO

- [x] 그리드 ↔ 리스트 뷰 전환 정상 동작
- [x] 뷰 전환 시 필터/검색 상태 유지 확인
- [x] 인라인 편집 후 저장 정상 동작
- [x] 인라인 편집 Esc 취소 확인
- [x] Batch 업데이트 API 정상 동작
- [x] 리스트 뷰에서 드래그앤드롭 순서 변경 확인
- [x] localStorage에 뷰 모드 선호 저장/복원 확인
...
- [x] **Sort Order Batch Update API** — `PUT /admin/menus/reorder`
  - body: `[{ menuId: 1, sortOrder: 0 }, { menuId: 3, sortOrder: 1 }, ...]`
  - 카테고리 내 또는 전체 메뉴의 sortOrder를 일괄 업데이트
- [x] **Service** — `AdminMenuService.reorderMenus(List<ReorderRequest>)`
  - 각 메뉴의 `sortOrder` 업데이트

### FE 체크리스트

- [x] **드래그 핸들 UI** — 메뉴 카드 이미지 좌상단에 이동 아이콘 (☰ 또는 ⠿)
...
- [x] 드래그 핸들을 통해서만 드래그 시작 확인
- [x] 카드 외 영역 드래그 시 미동작 확인
- [x] 드래그 후 정렬 순서 서버 저장 확인
- [x] 페이지 새로고침 후 변경된 순서 유지 확인
- [x] 카테고리 필터 적용 중 드래그 정상 동작
...
- [x] **품절 토글 API 확인/추가** — `PATCH /admin/menus/{id}/sold-out`
  - body: `{ isSoldOut: true/false }`
  - 기존 API에 포함되어 있는지 확인 → 없으면 추가
- [x] **기존 메뉴 목록 API 응답에 `isSoldOut` 포함 여부 확인**
...
- [x] 품절 토글 API 정상 동작 확인
- [x] 품절 상태 변경 후 UI 즉시 반영 확인
- [x] 품절 메뉴가 사용자 페이지에서 "품절" 표시 확인
- [x] 품절 메뉴 주문 시 주문 불가 처리 확인
...
- [x] `./gradlew bootJar -x test` 성공 확인
- [x] Docker 빌드 성공 확인 (`docker compose up -d --build backend`)
- [x] 서버 정상 기동 확인 (로그에 에러 없음)
- [x] Hibernate DDL 자동 업데이트 정상 확인 (새 테이블/컬럼 생성)

### API 테스트 (Phase별)

#### Phase 1: 공지 팝업

- [x] `POST /admin/notice-popups` — 팝업 생성
- [x] `GET /admin/notice-popups` — 전체 목록 조회
- [x] `PUT /admin/notice-popups/{id}` — 수정
- [x] `DELETE /admin/notice-popups/{id}` — 삭제
- [x] `GET /notice-popups/active` — 활성 팝업 조회 (비인증)

#### Phase 2: 등급 컬러

- [x] `PUT /admin/grade-settings/{grade}` — 컬러 포함 저장
- [x] `GET /admin/grade-settings` — 컬러 포함 응답 확인
- [x] `GET /grades/public` — 등급 시스템 활성 시 목록 반환
- [x] `GET /grades/public` — 등급 시스템 비활성 시 빈 반환

#### Phase 3: 주문 알림

- [x] `GET /admin/orders/subscribe` — SSE 연결 성공 (또는 polling 엔드포인트)
- [x] 새 주문 생성 후 SSE 이벤트 수신 확인

#### Phase 4: 메뉴 리스트 뷰

- [x] `PUT /admin/menus/batch` — 배치 업데이트 정상 동작

#### Phase 5: 메뉴 드래그앤드롭

- [x] `PUT /admin/menus/reorder` — sortOrder 일괄 업데이트

#### Phase 6: 메뉴 품절

- [x] `PATCH /admin/menus/{id}/sold-out` — 품절 토글
...
- [x] **공지 팝업**: 어드민에서 팝업 생성 → 사용자 페이지 접속 → 팝업 표시 → 오늘 하루 안 보기 → 새로고침 시 미표시
- [x] **등급 컬러**: 어드민에서 등급 컬러 설정 → 마이페이지 등급 카드 컬러 반영 → About 페이지 등급 섹션 컬러 반영
- [x] **등급 미사용**: 등급 시스템 비활성화 → About 페이지 등급 섹션 미노출 + API 미호출
- [x] **주문 알림**: 사용자 주문 → 어드민 벨 아이콘 깜빡임 + 토스트 표시 → 벨 클릭 → 주문 페이지 이동
- [x] **메뉴 리스트 뷰**: 뷰 전환 → 인라인 편집 → 저장 → 그리드 뷰로 전환 → 변경 사항 반영 확인
- [x] **메뉴 드래그앤드롭**: 그리드 뷰에서 핸들 드래그 → 순서 변경 → 새로고침 → 순서 유지 확인
- [x] **메뉴 품절**: 품절 토글 → 사용자 페이지에서 품절 표시 → 품절 메뉴 장바구니 담기 시 불가 확인
