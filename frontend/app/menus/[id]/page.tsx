...
export default function PublicMenuDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const {
        menu, optionsData, isLoading, error, isStoreOpen,
        selectedOptions, quantity, totalPrice, isOrderable,
        handleOptionChange, handleQuantityChange, handleAddToCart
    } = useMenuDetail(id);

    const handleAddToCartSuccess = () => {
        if (handleAddToCart()) setShowModal(true);
    };
    ...
    const handleAddFavorite = async () => {
        ...
        Object.entries(selectedOptions).forEach(([groupId, itemIds]) => {
            (itemIds as number[]).forEach(itemId => {
                mappedOptions.push({ optionGroupId: Number(groupId), optionItemId: Number(itemId) });
            });
        });
        try {
            await userFavoriteAPI.addFavorite({ menuId: menu.id, alias, selectedOptions: mappedOptions });
        ...
        <div className={styles.wrapper}>
            ...
            <main className={styles.container}>
                ...
                <div className={styles.mainContent}>
                    <ImageGallery images={menu?.images || []} menuName={menu.korName} />
                    ...
                    <div className={styles.rightContent}>
                        ...
                        <div className={styles.detailActions}>
                            ...
                            <MenuOptions optionsData={optionsData!} selectedOptions={selectedOptions} onOptionChange={handleOptionChange} />
                            ...
                            <div className={styles.orderArea}>
                                <div className={styles.quantityControl}>
                                    <div>
                                        ...
                                        <input type="number" min="1" value={quantity} onChange={(e) => handleQuantityChange(parseInt(e.target.value) || 1)} className={styles.quantityInput} />
                                    </div>
                                </div>
                                {isLoggedIn && (
                                    <button className={styles.favoriteBtn} onClick={handleAddFavorite} title="즐겨찾기 추가">
                                        <Star size={24} color="white" fill="white" />
                                    </button>
                                )}
                                <button className={styles.addToCartBtn} disabled={!isOrderable || menu.isSoldOut || !isStoreOpen} onClick={handleAddToCartSuccess}>
            ...
            {showModal && <CartModal menuName={menu.korName} onClose={() => setShowModal(false)} />}
            <Footer />
        </div>
    );
}
