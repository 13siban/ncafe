...
interface PointSectionProps {
    ...
    gradeInfo?: any;
    view?: 'all' | 'balance' | 'history';
}

const PointSection: React.FC<PointSectionProps> = ({ pointData, gradeInfo, view = 'all' }) => {
    return (
        <section className={styles.section}>
            {(view === 'all' || view === 'balance') && (
                <>
                    <h2 className={styles.sectionTitle}>내 포인트</h2>

                    <div className={styles.gradeCard} style={{ background: gradeInfo?.mainColor ? `linear-gradient(135deg, ${gradeInfo.mainColor}, ${gradeInfo.mainColor}dd)` : 'linear-gradient(135deg, var(--color-primary-500), var(--color-primary-700))', color: gradeInfo?.textColor || 'white', marginBottom: view === 'all' ? '24px' : '0' }}>
            ...
            {(view === 'all' || view === 'history') && (
                <>
                    <h2 className={styles.sectionTitle}>{view === 'history' ? '포인트 이용 내역' : ''}</h2>
                    {view === 'all' && <h3 style={{ fontSize: '1.1rem', fontWeight: 600, marginBottom: '16px', color: 'var(--color-gray-800)' }}>포인트 이용 내역</h3>}
